...
class StarGAN_v2():
    ...
    def build_model(self):
        if self.phase == 'train':
            ...
            self.s_optimizer = tf.keras.optimizers.Adam(learning_rate=self.lr, beta_1=self.beta1, beta_2=self.beta2, epsilon=1e-08)
            ...
            self.ckpt = tf.train.Checkpoint(generator=self.generator, generator_ema=self.generator_ema,
                                            ...
                                            discriminator=self.discriminator,siamese=self.siamese,
                                            g_optimizer=self.g_optimizer, e_optimizer=self.e_optimizer, f_optimizer=self.f_optimizer,
                                            d_optimizer=self.d_optimizer,s_optimizer=self.s_optimizer)
    ...
    @tf.function
    def g_train_step(self, x_real, y_org, y_trg, x_real2, z_trgs=None, x_refs=None):
        with tf.GradientTape(persistent=True) as g_tape:
            ...
            x_fake3 = self.generator([x_real2, s_trg])
            ...
            sab1=self.siamese(x_real2)
            sab2=self.siamese(x_fake3)
        ...
        s_train_variable=self.siamese.trainable_variables
        s_gradient = g_tape.gradient(ss_loss, s_train_variable)
        self.s_optimizer.apply_gradients(zip(s_gradient,s_train_variable))
    ...
    def train(self):
        ...
        for idx in range(self.start_iteration, self.iteration):
            ...
            x_real, x_real2, y_org = next(self.img_and_domain_iter)
    ...
    def refer_canvas(self, x_real, x_ref, y_trg, path, img_num):
        ...
        #     canvas.paste(PIL.Image.fromarray(np.uint8(dst_image), 'RGB'), (0, (row + 1) * self.img_size + 10))
            ...
            row_fake_images = self.generator_ema([x_real, s_trg])
            ...
            totalfake.append(row_fake_images)
        np.savez(path,norm_A=totalfake)
        # np.savez(os.path.join(self.dataset_path, domain)+'/'+str(i)+'-'+str(j)+'.npz', norm_A = specmain[j*80:((j+1)*80)-1])
    ...
    def test(self, merge=True, merge_size=0):
        ...
        for idx, domain in enumerate(self.domain_list):
            image_list = glob(os.path.join(reference_path, domain) + '/*.wav')
        ...
        if merge:
            ...
            if merge_size == 0:
                ...
                save_path = './{}/ref_all.npz'.format(self.result_dir)
                            ...
                            for ref_idx, (ref_img_path, ref_img_domain_) in enumerate(
                                ...
                                if ref_size < merge_size:
                                    ...
                                    if ref_size == merge_size:
                                        ...
                                        save_path = './{}/ref_{}_{}.npz'.format(self.result_dir, src_idx + 1, ref_idx + 1)
