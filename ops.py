...
def mod_loss(x,y):
    def my_func(xi,yi):
      xx=librosa.feature.mfcc(S=librosa.power_to_db(xi))
      yy=librosa.feature.mfcc(S=librosa.power_to_db(yi))
      
      # return metrics.melcd(xx,yy).astype('float32')
      return  np.sqrt(np.mean((xx-yy)**2))
    loss = tf.compat.v1.py_func(my_func, [x,y], tf.float32)
    return loss
